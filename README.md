...
- **MTP 投机解码**（单选）：无 / MTP / MTP2 / MTP3 / MTP4，对应 `--spec-type draft-mtp --spec-draft-n-max <1..4>`（预测 token 数，越高提速越明显），重启对应服务后生效。
...
- **打开 DSH 会话**：WebView2 嵌入 DSH Web（`http://127.0.0.1:3080/`），继承 markdown / 工具闭环 / 插件。
  - **自动校准模型配置**：打开会话时会先把 `settings.yaml` 的 `llm-pi-ai.providers.<该服务 provider>` 校准为托盘服务的实际端口与模型路径（`settingsYamlPath` 指向的文件；端口/模型名变了会自动修正，provider 块缺失会补插），再把 `agent-default-model` 临时指向该模型（8s 后还原）。
...
运行时状态（GPU 选择、ctx、KV 缓存、缓存内存、参数组、切分模式、MTP、监听地址）存 `dsh-tray.cfg`（`paramMode` / `gpu` / `ctx` / `kv` / `cacheRam` / `split` / `mtpLevel` / `bind`），自动读写。
